...
def test_execute_spawns_runner_container(mock_select, mock_docker_client):
    ...
    assert call_kwargs["volumes"] == {"/tmp/sim": {"bind": "/sandbox", "mode": "ro"}}
...
def test_execute_forwards_stdin_to_attach_socket(_mock_select, mock_docker_client):
    client, container, attach, sock = mock_docker_client
    sock.fileno.return_value = 1
    container.reload.side_effect = lambda: setattr(container, "status", "exited")
...
@patch("execution.select.select", return_value=([], [], []))
def test_execute_stop_kills_container(_mock_select, mock_docker_client):
    client, container, attach, sock = mock_docker_client
    sock.fileno.return_value = 1
...
def test_execute_timeout_kills_container(mock_monotonic, _mock_select, mock_docker_client):
